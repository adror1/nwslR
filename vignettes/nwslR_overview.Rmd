...
###Functionality
* `adv_team_stats` scrapes the NWSL website and returns over 200 variables about team performance boxscores and advanced statistics (2016-2019).
...
rookie_winners <- award %>%
  filter(award == "Rookie of the Year", 
         season >= 2015)
...
```{r}
rookie_stats <- left_join(rookie_winners, fieldplayer_overall_season_stats, by = c("person_id", "season")) 
...
```
 ...
 First, we need to join our `team_id` column to the `franchise` dataset, so ensure functionality with the `teamcolors` dataset for visualization. This dataset uses the full team name rather than an ID. 
```{r}
rookie_team <- left_join(rookie_stats, franchise, by = c("team_id", "season"))
...
ggplot(rookie_team, aes(x = reorder(player, season), y = gls, fill = team_name)) +
  geom_bar(stat = "identity") +
  scale_fill_teams(2) +
  geom_text(aes(label = season), position = position_dodge(width = 0.9), vjust = -0.25) +
  labs(x = "Player",
       y = "Goals Scored",
...
```{r}
stats_2017 <- get_adv_team_stats(2017)
...
```{r, fig.height=5, fig.width=7}
ggplot(stats_name, aes(x = game_date, y = total_pass, group = team_id, color = team_name)) +
  geom_line() +
  scale_color_teams(1) +
